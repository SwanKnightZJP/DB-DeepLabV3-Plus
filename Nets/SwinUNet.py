...
class SwinUnet(nn.Module):
    def __init__(self, num_classes=3, img_size=(224,224), patch_size=(4,4), zero_head=False, vis=False):
    ...
    def forward(self, x):
        ...
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        logits = self.swin_unet(x)   # x: B=16, C=3, H=128, W=512

        if x.size()[2] == 256:
            logits = F.pad(logits, (0, 0, 16, 16), "constant", 0)

        return logits

    # # pretrained model disabled
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = SwinUnet(num_classes=3, img_size=(256,512), patch_size=(4, 4)).to(device)
